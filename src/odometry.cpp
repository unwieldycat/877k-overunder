...
float Odometry::local_to_global_coords(float local_x, float local_y, float robot_heading, bool return_x = true) {
...
float Odometry::global_to_local_coords(float global_x, float global_y, float robot_x, float robot_y, float robot_heading, bool return_x = true) {
...
void Odometry::track_position() {
	...
	while (true) {
		...
		tracking_x += local_to_global_coords(localXDist, localYDist, imu_heading, true);
		tracking_y += local_to_global_coords(localXDist, localYDist, imu_heading, false);

		odom_x = tracking_x + local_to_global_coords(RIGHT_OFFSET - LEFT_OFFSET, REAR_OFFSET, imu_heading, true);
		odom_y = tracking_y + local_to_global_coords(RIGHT_OFFSET - LEFT_OFFSET, REAR_OFFSET, imu_heading, false);
